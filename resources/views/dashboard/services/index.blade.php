            ...
            <table class="table table-bordered table-striped">
                ...
                @foreach ($services as $service)
                    <tr>
                        <td>{{ $service->id }}</td>
                        <td>{{ $service->name }}</td>
                        <td>{{ $service->serviceType->name ?? 'Не указано' }}</td>
                        <td>{{ $service->description }}</td>
                        <td>{{ number_format($service->price, 2) }} ₽</td>
                        <td class="text-end">
                            <a href="{{ route('dashboard.services.show', $service) }}" class="btn btn-sm btn-info">Просмотр</a>
                            <a href="{{ route('dashboard.services.edit', $service) }}" class="btn btn-sm btn-warning">Редактировать</a>
