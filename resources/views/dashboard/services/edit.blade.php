...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Редактирование услуги</h3>
        </div>
        <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-group mb-3">
                        ...
                        <select name="service_type_id" id="service_type_id" class="form-multi-select" data-coreui-multiple="false" data-coreui-search="true" data-coreui-options-style="text" required>
                            ...
                            @foreach ($serviceTypes as $id => $name)
                                <option value="{{ $id }}" {{ old('service_type_id', $service->service_type_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 form-group mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="price" id="price" step="0.01" min="0" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $service->price) }}" required>
                    ...
                    <div class="col-md-12 form-group mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
